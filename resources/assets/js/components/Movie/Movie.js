...
export class Movie extends React.Component {
  ...
  render(){
    ...
    const columns = [{
      ...
      render: (text, record) => (
        <span>
            <Link to={'/movies/update/' + record.id}>
              {record.title}
            </Link>
        </span>
      )
        ...
        <div style={{overflow:'hidden'}}>
          ...
          <Dropdown
            ...
            overlay={
              <Menu>
                <Menu.Item>
                  <Link to="/movies/create/richtext">
                    <Icon type="file-word" /> 富文本编辑器
                  </Link>
                </Menu.Item>
                <Menu.Item>
                  <Link to="/movies/create/markdown">
  ...
  fetchData = (currentPage=null, pageSize=null) =>{
    ...
    let url = window.apiURL + 'movies?order=' + this.state.order + '&pagesize=' + pageSize + '&page=' + currentPage;
  ...
  handlePublish = (id) =>{
    var that = this
    axios.get(window.apiURL + 'movies/publish/' + id)
  ...
  handleTop = (id) =>{
    var that = this
    axios.get(window.apiURL + 'movies/top/' + id)
    .then(function (response) {
      if (response.status == 200) {
        that.fetchData()
        message.success(response.data.message)
  ...
  handleDelete = (id) =>{
    ...
    confirm({
      ...
      onOk() {
        //获取文章数据
        axios.get(window.apiURL + 'movies/delete/' + id)
        .then(function (response) {
          if (response.status == 200) {
            that.fetchData()
            message.success(response.data.message)
